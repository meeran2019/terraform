...
resource "docker_container" "busybox-container" {
  ...
  volumes {
    container_path = "/tmp"
    host_path      = "/home/meeran/Desktop/terraform/terraform-tutorial/tf-variables/hostvolume"
  }
  command = [
    "sleep",
    "3700"
...
# null resource can be used to run a command. 
# || true is used to suppress the error. 

resource "null_resource" "host-volume" {
  provisioner "local-exec" {
    command = "mkdir hostvolume || true"
  }
}
